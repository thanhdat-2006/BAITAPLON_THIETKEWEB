...
// ======================================================
// 4. LOGIC TRANG TOURS (Load danh sách Tour)
// ======================================================
async function loadToursPage() {
    try {
        ...
        container.innerHTML = '';
        tours.forEach(tour => {
            // Lưu ý: Nút button có class 'btn-book' để Modal bắt sự kiện
            container.innerHTML += `
                <div class="tour-card">
                    <div class="tour-img">
                    ...
                    <div class="tour-details">
                        <h4><i class="fas fa-map-pin"></i> ${tour.name}</h4>
                        <div class="tour-meta">
                            <i class="fas fa-calendar-days"></i> ${tour.time}
                        </div>
                        <div class="tour-price"><i class="fas fa-money-bill-1"></i> ${tour.price}</div>
                        <button class="btn-book" type="button">Đặt Ngay</button>
                    </div>
                </div>`;
        });
    } catch (err) {
...
function setupBookingModal() {
    ...
    document.addEventListener('click', (e) => {
        if (e.target && e.target.classList.contains('btn-book')) {
            ...
            if (card) {
                currentTourName = card.querySelector('h4').innerText;
                const price = card.querySelector('.tour-price').innerText;
    ...
    // Thay thế đoạn code xử lý sự kiện submit trong setupBookingModal (khoảng dòng 270)
if (form) {
    form.addEventListener('submit', (e) => {
        ...
        const bookingInfo = {
            ...
            tourName: String(currentTourName || ''),
...
async function loadTourDetail() {
    try {
        ...
        if (!id) {
            document.getElementById('tour-detail-container').innerHTML = '<p style="text-align:center; margin-top:50px">Không tìm thấy tour.</p>';
            return;
        }
        ...
        if (!tour) {
            document.getElementById('tour-detail-container').innerHTML = '<p style="text-align:center; margin-top:50px">Tour không tồn tại.</p>';
            return;
        }

        const container = document.getElementById('tour-detail-container');
        container.innerHTML = `
            <div class="detail-header" style="background: url('${tour.image}') no-repeat center center/cover;">
                <div class="container header-content">
                    <h1>${tour.name}</h1>
                    <p style="font-size: 1.2rem; color: #fff;">${tour.time}</p>
                </div>
            </div>
            ...
            <div class="container" style="padding: 50px 0;">
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 40px;">
                    <div class="main-content">
                        <div class="detail-box">
                            <h3><i class="fas fa-info-circle"></i> Chi Tiết Tour</h3>
                            <p>Tour du lịch tuyệt vời với các điểm tham quan nổi tiếng nhất. Thời gian: <strong>${tour.time}</strong></p>
                            <p>Giá vé: <strong style="color: #e74c3c; font-size: 1.3rem;">${tour.price}</strong></p>
                        </div>

                        <div class="detail-box">
                            <h3><i class="fas fa-camera-retro"></i> Lịch Trình</h3>
                            <p>Tour bao gồm các hoạt động thú vị, khám phá địa danh nổi tiếng và trải nghiệm văn hóa địa phương.</p>
                            <ul style="margin-left: 20px; margin-top: 15px;">
                                <li>Hướng dẫn viên tiếng Việt chuyên nghiệp</li>
                                <li>Thực phẩm và đồ uống khi cần thiết</li>
                                <li>Các bảo hiểm du lịch</li>
                                <li>Vận chuyển thoải mái</li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class="sidebar">
                        <div class="detail-box">
                            ...
                            <p style="font-size: 1.5rem; color: #f39c12;">★★★★★</p>
                            <p>4.9/5 từ 500+ khách hàng</p>
                        </div>
                        <div class="detail-box">
                            <h3><i class="fas fa-calendar"></i> Ngày Khượi Hành</h3>
                            <p>Tour khởi hành hàng tuần</p>
                            <button class="btn-book" style="margin-top: 15px; width: 100%; padding: 12px;">Đặt Tour Ngay</button>
                        </div>
                    </div>
                </div>
            </div>
...
// ======================================================
// 8. LOGIC TRANG HOTELS (Load danh sách & Bộ lọc)
// ======================================================
...
// ======================================================
// 9. LOGIC TRANG CHI TIẾT HOTEL (ĐÃ CẬP NHẬT GỌI MODAL ĐẶT PHÒNG)
// ======================================================
...
// ======================================================
// 11. MODAL HOTEL (Mở chi tiết hotel trong modal)
// ======================================================
...
// ======================================================
// 12. LOGIC TRANG CHI TIẾT TOUR (UPDATE HOÀN CHỈNH)
// ======================================================
    ...
    fetch('assets/data/tours.json')
      ...
      .then(list => {
        if (!Array.isArray(list) || list.length === 0) {
          container.innerHTML = '<p style="text-align:center">Không có tour.</p>';
          return;
        }
        container.innerHTML = '';
        list.forEach(t => {
          const id = t.id !== undefined ? t.id : (t.name ? encodeURIComponent(t.name) : '');
          const link = `tour-detail.html?id=${encodeURIComponent(id)}`;
          const img = t.image ? escapeHtml(t.image) : 'assets/images/default.jpg';
          const name = escapeHtml(t.name || 'Tour');
          ...
          card.innerHTML = `
            <a class="tour-thumb" href="${link}" title="${name}">
              <img src="${img}" alt="${name}" style="width:100%;height:auto;display:block;border-radius:8px;object-fit:cover;">
            </a>
            <div class="tour-card-body">
              <h4 class="tour-card-title"><a href="${link}" style="text-decoration:none;color:inherit">${name}</a></h4>
              <div class="tour-meta" style="display:flex;justify-content:space-between;align-items:center;">
                <span class="time">${time}</span>
                <span class="price" style="color:#00b387;font-weight:700">${price}</span>
              </div>
            </div>
          `;
      ...
      .catch(err => {
        console.error(err);
        container.innerHTML = '<p style="text-align:center;color:#d32f2f">Lỗi tải dữ liệu tour.</p>';
      });
  }

  document.addEventListener('DOMContentLoaded', function () {
    try { initToursListPage(); } catch (e) {}
    // ...other inits...
  });
})();
